<?php
    class authDB
    {
        ...
        function __construct(private PDO $pdo){
            ...
            $this->statementReadSession = $pdo->prepare('SELECT * FROM session WHERE id=?');
            $this->statementReadUser = $pdo->prepare('SELECT * FROM user WHERE id=?');
            $this->statementReadUserFromEmail = $pdo->prepare('SELECT * FROM user WHERE email=?');
        ...
        function login(string $userId):void{
            ...
            $signature = hash_hmac('sha256',$sessionId,'ici est la phrase secrete');
        ...
        function isLoggedin():array |false{
            ...
            if($sessionId && $signature){
                $hash = hash_hmac('sha256',$sessionId,'ici est la phrase secrete');
                if(hash_equals($hash,$signature)){
                    $this->statementReadSession->execute([$sessionId]);
                    $session = $this->statementReadSession->fetch();
                    if($session){

                        $this->statementReadUser->execute([$session['userid']]);
        ...
        function getUserFromEmail(string $email):array | false{
            $this->statementReadUserFromEmail->execute([$email]);
